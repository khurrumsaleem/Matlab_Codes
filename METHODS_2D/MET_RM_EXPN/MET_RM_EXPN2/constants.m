 ...
 for ry = 1: RY
   ...
   for j = 1: ncy
     ...
     for rx = 1: RX
       ...
       for i = 1: ncx
         IB = IB + 1;
         XN0(JB, IB) = 0.5 * xh * LANDA0(JB, IB) / sinh(0.5 * xh * LANDA0(JB, IB));
         XNB(JB, IB) = 0.5 * xh * LANDAB(JB, IB) / sinh(0.5 * xh * LANDAB(JB, IB));
         XNT(JB, IB) = 0.5 * xh * LANDAT(JB, IB) / sinh(0.5 * xh * LANDAT(JB, IB));
         
         YN0(JB, IB) = 0.5 * yh * LANDA0(JB, IB) / sinh(0.5 * yh * LANDA0(JB, IB));
         YNL(JB, IB) = 0.5 * yh * LANDAL(JB, IB) / sinh(0.5 * yh * LANDAL(JB, IB));
         YNR(JB, IB) = 0.5 * yh * LANDAR(JB, IB) / sinh(0.5 * yh * LANDAR(JB, IB));
       end
     end
   end
 end
