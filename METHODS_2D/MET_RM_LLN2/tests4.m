...
for m = 1: M
  ...
  for k = 1: M
    ...
    % x-1/2
    comp0 = miu*alfa(k)*landa(m,k)*exp(-0.5*st*hi/xvals(k,1))/xvals(k,1) + miu*alfa(k)*gama(m,k)*exp(-0.5*st*hi/xvals(k,1))/st ...
            + alfa(k)*landa(m,k)*exp(-0.5*st*hi/xvals(k,1)) - AUX0*exp(-0.5*st*hi/xvals(k,1)) - 6*theta*alfa(k)*xvects(m,k,1)*exp(-0.5*st*hi/xvals(k,1))/(st*hj);
    comp1 = -0.5*hi*miu*alfa(k)*gama(m,k)*exp(-0.5*st*hi/xvals(k,1))/xvals(k,1) -0.5*hi*alfa(k)*gama(m,k)*exp(-0.5*st*hi/xvals(k,1)) + 0.5*hi*AUX1*exp(-0.5*st*hi/xvals(k,1));
