...
function [R, S, F0, F1, SP] = response_matrix2(N, ZON, XDOM, YDOM, ZMAP, QMAP)
  ...
  M = N * (N + 2) / 2;
  R = zeros(4 * M, 4 * M, J, I); S = zeros(4 * M, J, I);
  F0 = zeros(M, M, J, I); F1 = zeros(M, M, J, I);
  SP = zeros(M, J, I);
      ...
      for rx = 1: NRX
        ...
        for i = 1: ncx
          ...
          for m = 1: M
            ...
            SP(m,JB,IB) = Q * (1 + c0 / (1 - c0)) / st;
            for k = 1: M
              k_miu = QUAD(k,1); k_theta = QUAD(k,2); k_w = QUAD(k,3);
              F0(m,k,JB,IB) = 0.25 * c0 * k_w * k_miu / (st * hx * (1-c0));
              F1(m,k,JB,IB) = 0.25 * c0 * k_w * k_theta / (st * hy * (1-c0));
              if (m == k)
                F0(m,k,JB,IB) = F0(m,k,JB,IB) + m_miu / (st * hx);
                F1(m,k,JB,IB) = F1(m,k,JB,IB) + m_theta / (st * hy);
              end
              if (k <= M/4)
                F0(m,k,JB,IB) = F0(m,k,JB,IB);
                F1(m,k,JB,IB) = F1(m,k,JB,IB);
              elseif (k > M/4 && k <= M/2)
                F0(m,k,JB,IB) = - F0(m,k,JB,IB);
                F1(m,k,JB,IB) = F1(m,k,JB,IB);
          ...
          for m = 1: M
            if (m <= M/4)
              for k = 1: M
                ...
                GAMA_XIN(m,k) = (IB - 1) * hx * xgama(m,k) * exp(- 0.5 * st * hx / xvals(k,z));
                GAMA_XOUT(m,k) = IB * hx * xgama(m,k) * exp(0.5 * st * hx / xvals(k,z));

                LAMBDA_YIN(m,k) = ylanda(m,k) * exp(- 0.5 * st * hy / yvals(k,z));
                LAMBDA_YOUT(m,k) = ylanda(m,k) * exp(0.5 * st * hy / yvals(k,z));
                GAMA_YIN(m,k) = (JB - 1) * hy * ygama(m,k) * exp(- 0.5 * st * hy / yvals(k,z));
                GAMA_YOUT(m,k) = JB * hy * ygama(m,k) * exp(0.5 * st * hy / yvals(k,z));
              end
            elseif (m > M/4 && m <= M/2)
              for k = 1: M
                LAMBDA_XIN(m,k) = xlanda(m,k) * exp(0.5 * st * hx / xvals(k,z));
                LAMBDA_XOUT(m,k) = xlanda(m,k) * exp(- 0.5 * st * hx / xvals(k,z));
                GAMA_XIN(m,k) = IB * hx * xgama(m,k) * exp(0.5 * st * hx / xvals(k,z));
                GAMA_XOUT(m,k) = (IB - 1) * hx * xgama(m,k) * exp(- 0.5 * st * hx / xvals(k,z));
            ...
            elseif (m > M/2 && m <= 3*M/4)
              for k = 1: M
                ...
                LAMBDA_YIN(m,k) = ylanda(m,k) * exp(0.5 * st * hy / yvals(k,z));
                LAMBDA_YOUT(m,k) = ylanda(m,k) * exp(- 0.5 * st * hy / yvals(k,z));
                GAMA_YIN(m,k) = JB * hy * ygama(m,k) * exp(0.5 * st * hy / yvals(k,z));
                GAMA_YOUT(m,k) = (JB - 1) * hy * ygama(m,k) * exp(- 0.5 * st * hy / yvals(k,z));
              end
            elseif (m > 3*M/4 && m <= M)
              for k = 1: M
                LAMBDA_XIN(m,k) = xlanda(m,k) * exp(- 0.5 * st * hx / xvals(k,z));
